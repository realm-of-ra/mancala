...
export default function Lobby() {
  ...
  const filteredGames = data?.mancalaChiMancalaBoardModels?.edges
    ?.filter(
      (game: any) =>
    ...
    .map((game: any) => {
      const player1Profile = playerData?.mancalaChiProfileModels?.edges?.find(
        (profile: any) => profile.node.address === game.node.player_one,
      );
      const player2Profile = playerData?.mancalaChiProfileModels?.edges?.find(
        (profile: any) => profile.node.address === game.node.player_two,
      );

      const winner = playerData?.mancalaChiProfileModels?.edges?.find(
  ...
  const filteredTransactions =
    data?.mancalaChiMancalaBoardModels?.edges?.reduce(
      (acc: any[], game: any) => {
        if (
          game?.node?.player_one === account?.address ||
          game?.node?.player_two === account?.address ||
  ...
  const gamesWithPlayerNames = data?.mancalaChiMancalaBoardModels?.edges
    ?.map((game: any) => {
      const player1Profile = playerData?.mancalaChiProfileModels?.edges?.find(
        (profile: any) => profile.node.address === game.node.player_one,
      );
  ...
  useEffect(() => {
    if (data?.mancalaChiMancalaBoardModels?.edges) {
      const allAddresses = data?.mancalaChiMancalaBoardModels.edges
        .flatMap((game: any) => [
  ...
  }, [data?.mancalaChiMancalaBoardModels.edges]);
