...
export default function Lobby() {
  ...
  const filteredGames = data?.mancalaAvalonMancalaBoardModels?.edges
    ?.filter(
      (game: any) =>
    ...
    .map((game: any) => {
      const player1Profile =
        playerData?.mancalaAvalonProfileModels?.edges?.find(
      ...
      const winner = playerData?.mancalaAvalonProfileModels?.edges?.find(
  ...
  const filteredTransactions =
    data?.mancalaAvalonMancalaBoardModels?.edges?.reduce(
      (acc: any[], game: any) => {
        if (
          game?.node?.player_one === account?.address ||
          game?.node?.player_two === account?.address ||
  ...
  const gamesWithPlayerNames = data?.mancalaAvalonMancalaBoardModels?.edges
    ?.map((game: any) => {
      const player1Profile =
        playerData?.mancalaAvalonProfileModels?.edges?.find(
          (profile: any) => profile.node.address === game.node.player_one,
  ...
  useEffect(() => {
    if (data?.mancalaAvalonMancalaBoardModels?.edges) {
      const allAddresses = data?.mancalaAvalonMancalaBoardModels.edges
        .flatMap((game: any) => [
  ...
  }, [data?.mancalaAvalonMancalaBoardModels.edges]);
