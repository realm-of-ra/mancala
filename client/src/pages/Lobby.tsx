...
export default function Lobby() {
  ...
  const filteredGames = data?.mancalaKefirMancalaBoardModels?.edges
    ?.filter(
      (game: any) =>
    ...
    .map((game: any) => {
      const player1Profile = playerData?.mancalaKefirProfileModels?.edges?.find(
        (profile: any) => profile.node.address === game.node.player_one,
      );
      const player2Profile = playerData?.mancalaKefirProfileModels?.edges?.find(
        (profile: any) => profile.node.address === game.node.player_two,
      );

      const winner = playerData?.mancalaKefirProfileModels?.edges?.find(
  ...
  const filteredTransactions =
    data?.mancalaKefirMancalaBoardModels?.edges?.reduce(
      (acc: any[], game: any) => {
        if (
          game?.node?.player_one === account?.address ||
          game?.node?.player_two === account?.address ||
  ...
  const gamesWithPlayerNames = data?.mancalaKefirMancalaBoardModels?.edges
    ?.map((game: any) => {
      const player1Profile = playerData?.mancalaKefirProfileModels?.edges?.find(
        (profile: any) => profile.node.address === game.node.player_one,
      );
  ...
  useEffect(() => {
    if (data?.mancalaKefirMancalaBoardModels?.edges) {
      const allAddresses = data?.mancalaKefirMancalaBoardModels.edges
        .flatMap((game: any) => [
  ...
  }, [data?.mancalaKefirMancalaBoardModels.edges]);
