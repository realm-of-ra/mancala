...
export default function Header() {
  ...
  useEffect(() => {
    const profile: any = playerData?.mancalaMondayProfileModels?.edges.find(
  ...
  }, [account, playerData?.mancalaMondayProfileModels?.edges, playerData]);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const userExists = profiles.mancalaMondayProfileModels.edges.some(
  ...
  useEffect(() => {
    if (
      account?.address &&
      playerData?.mancalaMondayProfileModels?.edges &&
      !profile
    ) {
      setOpen(true);
    }
