...
const GameBoard: React.FC<GameBoardProps> = ({
  ...
  const involved = game_players?.mancalaMondayPlayerModels.edges.some(
    (item: any) =>
      item?.node.address ===
      (account.account?.address || game_node?.player_one),
  );
  const player_position = involved
    ? game_players?.mancalaMondayPlayerModels.edges.findIndex(
        (item: any) =>
          item?.node.address ===
          (account.account?.address || game_node?.player_one),
      )
  ...
  useEffect(() => {
    const captures = captureData?.mancalaMondayCaptureModels?.edges;
  ...
  useEffect(() => {
    const extraTurns = extraTurnData?.mancalaMondayPlayerExtraTurnModels?.edges;
    if (
      extraTurns &&
      extraTurns.length > 0 &&
  ...
  const seeds = React.useMemo(() => {
    if (!data?.mancalaMondaySeedModels?.edges) return [];
    const uniqueSeeds = new Map();

    // Sort edges by timestamp in descending order (newest first)
    const sortedEdges = [...data.mancalaMondaySeedModels.edges].sort((a, b) => {
  ...
  useEffect(() => {
    if (seeds.length > 0 && selectedPit !== null && isPlayerTurn) {
      ...
      const player =
        game_players?.mancalaMondayPlayerModels.edges[player_position]?.node
          .address;
      const opponent =
        game_players?.mancalaMondayPlayerModels.edges[opponent_position]?.node
          .address;
  ...
  useEffect(() => {
    if (data?.mancalaMondaySeedModels?.edges) {
  ...
  const player_pot_seed_count =
    isSimulating && simulatedSeeds.length > 0
      ...
      : game_players?.mancalaMondayPitModels.edges
          .filter(
            (item: any) =>
              item?.node.player ===
              game_players?.mancalaMondayPlayerModels.edges[player_position]
                ?.node.address,
          )
    ...
    isSimulating && simulatedSeeds.length > 0
      ...
      : game_players?.mancalaMondayPitModels.edges
          .filter(
            (item: any) =>
              item?.node.player ===
              game_players?.mancalaMondayPlayerModels.edges[opponent_position]
                ?.node.address,
          )
          .filter((item: any) => item?.node.pit_number === 7)[0]?.node
          ?.seed_count || 0;
            ...
            <div className="flex flex-row justify-center flex-1 items-center w-[100px] space-x-5">
              {game_players?.mancalaMondayPitModels.edges
                .filter(
                  (item: any) =>
                    item?.node.player ===
                    game_players?.mancalaMondayPlayerModels.edges[
                      opponent_position
                    ]?.node.address,
                )
