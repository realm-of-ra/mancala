...
const GameBoard: React.FC<GameBoardProps> = ({
  ...
  const involved = game_players?.mancalaKefirPlayerModels.edges.some(
    (item: any) =>
      item?.node.address ===
      (account.account?.address || game_node?.player_one),
  );
  const player_position = involved
    ? game_players?.mancalaKefirPlayerModels.edges.findIndex(
        (item: any) =>
          item?.node.address ===
          (account.account?.address || game_node?.player_one),
      )
  ...
  useEffect(() => {
    const captures = captureData?.mancalaKefirCaptureModels?.edges;
  ...
  useEffect(() => {
    const extraTurns = extraTurnData?.mancalaKefirPlayerExtraTurnModels?.edges;
    if (
      extraTurns &&
      extraTurns.length > 0 &&
  ...
  const seeds = React.useMemo(() => {
    if (!data?.mancalaKefirSeedModels?.edges) return [];
    const uniqueSeeds = new Map();

    // Sort edges by timestamp in descending order (newest first)
    const sortedEdges = [...data.mancalaKefirSeedModels.edges].sort((a, b) => {
  ...
  useEffect(() => {
    if (seeds.length > 0 && selectedPit !== null && isPlayerTurn) {
      ...
      const player =
        game_players?.mancalaKefirPlayerModels.edges[player_position]?.node
          .address;
      const opponent =
        game_players?.mancalaKefirPlayerModels.edges[opponent_position]?.node
          .address;
  ...
  useEffect(() => {
    if (data?.mancalaKefirSeedModels?.edges) {
  ...
  const player_pot_seed_count =
    isSimulating && simulatedSeeds.length > 0
      ? calculatePitAmount(
          simulatedSeeds,
          7,
          game_players?.mancalaKefirPlayerModels.edges[
            involved ? player_position : opponent_position
          ]?.node.address,
        )
      : game_players?.mancalaKefirPitModels.edges
          .filter(
            (item: any) =>
              item?.node.player ===
              game_players?.mancalaKefirPlayerModels.edges[
