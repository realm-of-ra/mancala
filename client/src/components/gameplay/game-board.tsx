...
const GameBoard: React.FC<GameBoardProps> = ({
  ...
  const involved = game_players?.mancalaChiPlayerModels.edges.some(
    (item: any) =>
      item?.node.address ===
      (account.account?.address || game_node?.player_one),
  );
  const player_position = involved
    ? game_players?.mancalaChiPlayerModels.edges.findIndex(
        (item: any) =>
          item?.node.address ===
          (account.account?.address || game_node?.player_one),
      )
  ...
  useEffect(() => {
    const captures = captureData?.mancalaChiCaptureModels?.edges;
  ...
  useEffect(() => {
    const extraTurns = extraTurnData?.mancalaChiPlayerExtraTurnModels?.edges;
    if (
      extraTurns &&
      extraTurns.length > 0 &&
  ...
  const seeds = React.useMemo(() => {
    if (!data?.mancalaChiSeedModels?.edges) return [];
    const uniqueSeeds = new Map();

    // Sort edges by timestamp in descending order (newest first)
    const sortedEdges = [...data.mancalaChiSeedModels.edges].sort((a, b) => {
  ...
  useEffect(() => {
    if (seeds.length > 0 && selectedPit !== null && isPlayerTurn) {
      ...
      const player =
        game_players?.mancalaChiPlayerModels.edges[player_position]?.node
          .address;
      const opponent =
        game_players?.mancalaChiPlayerModels.edges[opponent_position]?.node
          .address;
  ...
  useEffect(() => {
    if (data?.mancalaChiSeedModels?.edges) {
  ...
  const player_pot_seed_count =
    isSimulating && simulatedSeeds.length > 0
      ? calculatePitAmount(
          simulatedSeeds,
          7,
          game_players?.mancalaChiPlayerModels.edges[
            involved ? player_position : opponent_position
          ]?.node.address,
        )
      : game_players?.mancalaChiPitModels.edges
          .filter(
            (item: any) =>
              item?.node.player ===
              game_players?.mancalaChiPlayerModels.edges[
  ...
  const opponent_pot_seed_count =
    isSimulating && simulatedSeeds.length > 0
      ? calculatePitAmount(
          ...
          7,
          game_players?.mancalaChiPlayerModels.edges[
            involved ? opponent_position : player_position
          ]?.node.address,
        )
      : game_players?.mancalaChiPitModels.edges
          .filter(
            ...
            <div className="flex flex-row justify-center flex-1 space-x-5">
              {game_players?.mancalaChiPitModels.edges
                .filter(
                  (item: any) =>
                    item?.node.player ===
                    game_players?.mancalaChiPlayerModels.edges[player_position]
                      ?.node.address,
                )
                .filter((item: any) => item?.node.pit_number !== 7)
