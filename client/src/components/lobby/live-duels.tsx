...
import DuelLoaderA from "@/components/ui/svg/DuelLoaderA.tsx";
...
export default function LiveDuels({games, transactions}: { games: any, transactions: any }) {
    ...
    const runGameAction = (index: number) => {
        if (games[index].node.player_one === account.account.address || games[index].node.player_two === account.account.address) {
    ...
    if (!data) {
        return <div className="w-[874px] h-[437px] flex flex-col items-center justify-center">
            <svg className="text-white animate-spin w-fit" viewBox="0 0 64 64" fill="none"
                ...
                <path
                    ...
                    stroke="#FCE3AA" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"
